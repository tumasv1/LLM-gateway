...
- ✅ Трейсинг в Langfuse: проект `llm-gateway` (не RAGv2), `callbacks: ["langfuse_otel"]` (OTLP на `{LANGFUSE_HOST}/api/public/otel`) + хук `hooks/langfuse_key_alias_tag.py` (короткий тег `openclaw`/`openwebui`/`ragv2`, плюс `session_id`/`trace_user_id` из заголовков OpenWebUI). Ключи графа RAGv2 шлюзом не использовать. Правка yaml — `docker compose restart litellm`; смена `.env` — `docker compose up -d litellm`.
...
- **Presidio на ключ через UI/metadata.guardrails в OSS v1.100.0 нельзя** — при запросе 403 Enterprise. Вешаем только UI-guardrail с Default On выкл + хук по алиасу (`OpenWebUI-2`). Не писать `guardrails` в metadata ключа.
